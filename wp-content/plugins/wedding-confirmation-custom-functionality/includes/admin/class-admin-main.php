...
class Admin_Main {
	private const PAGE_SLUG = 'wedding-confirmations';

	private Confirmations_DB_Service $confirmations_service;

	public function __construct( Confirmations_DB_Service $confirmations_service ) {
	...
	function handle_confirmations_export_old() {
		$results = $this->confirmations_service->get_all_confirmations();
	...
	function add_admin_menu() {
		$menu_page_hook = add_menu_page(
			__( 'Wedding Confirmations', 'wccf_domain' ),
			__( 'Confirmations', 'wccf_domain' ),
			'manage_options',
			self::PAGE_SLUG,
		...
		add_action( "load-{$menu_page_hook}", array( $this, 'load_main_menu_page_assets' ) );
	}
	...
	public function render_search_form( string $search_term ): void {
		...
		echo '<input type="submit" value="' . __( 'Search', 'wccf_domain' ) . '" class="button button-primary submit-button"  style="margin-right: 10px;"/>';
		echo '</form>';
